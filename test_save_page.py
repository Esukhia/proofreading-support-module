...
from proofreading import update_index, get_old_layers, save_page, get_page
...
def test_update_index():
    ...
    vol_num = 1
    page_start = 300
    old_pecha_idx = load_yaml(Path('./test/data/old_index.yml'))
    new_pecha_idx = update_index(vol_offset, vol_num, page_start, old_pecha_idx)
    new_pecha_idx = to_yaml(new_pecha_idx)
    expected_idx = Path('./test/data/expected_index.yml').read_text(encoding='utf-8')
    assert new_pecha_idx == expected_idx


def test_update_layers():
    pecha_opf_path = Path('./test/data/P0003')
    pecha_id = "P0003"
    vol_id = 'v001'
    old_vol = (pecha_opf_path / f'P0003.opf/base/v001.txt').read_text(encoding='utf-8')
    new_vol = Path('./test/data/new_vol.txt').read_text(encoding='utf-8')
    ...
    for layer_name, old_layer in old_layers.items():
        ...
        expected_layer = load_yaml(Path(f'./test/data/expected_layers/{layer_name}.yml'))
